...
namespace RolePermission.BLL
{
    public partial class SMMENUTBService : BaseService<SMMENUTB>, ISMMENUTBService
    {
        ...
        public string GetMenuByAccount(ref Account person)
        {
            ...
            var roleIds = this.GetCurrentDbSession.ISMUSERTBRepository.LoadEntities(x => x.USER_ID == personId).Include(c=>c.SMROLETB).FirstOrDefault().SMROLETB.Select(s => s.ROLE_ID).ToList();
        ...
        public SMMENUTB Create(SMMENUTB entity)
        {
            ...
            foreach (int item in entity.SysOperationId.GetIdSort())
            {
                ...
                entity.SMFUNCTB.Add(new MenuFunc()
                {
                    Func = sys,
                    Menu = entity,
                });
        ...
        public SMMENUTBService(IDBSession session) : base(session)
        {
        }
        public override void SetCurretnRepository()
        {
            CurrentRepository = GetCurrentDbSession.ISMMENUTBRepository;
        }
    }
}
